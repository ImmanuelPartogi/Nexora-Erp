// FILE: frontend/src/modules/data/product/pages/ProductListPage.tsx
...
function TH({ children, right }: { children: React.ReactNode; right?: boolean }) {
  return (
    <th style={{
      padding: '10px 16px', textAlign: right ? 'right' : 'left', fontSize: 10.5, fontWeight: 700,
      textTransform: 'uppercase' as const, letterSpacing: '0.1em', color: '#94a3b8',
      background: '#f8fafc', borderBottom: '1px solid #f1f5f9', whiteSpace: 'nowrap' as const, fontFamily: 'inherit',
    }}>
      {children}
    </th>
  );
...
export const ProductListPage = () => {
  ...
  const { data, isLoading, refetch } = useProducts({ page, limit: 10, search, type: type || undefined });
  const showToast = (msg: string) => { setToast(msg); setTimeout(() => setToast(null), 3000); };

  const handleCreate = async (formData: ProductFormData) => {
    try {
      setIsCreating(true); setErrorMsg('');
      ...
      if (err instanceof AxiosError) {
        ...
        const message = err.response?.data?.error ?? 'Gagal membuat produk.';
        if (status === 409) setErrorMsg(message);
        else if (status === 422) {
          const errs = err.response?.data?.errors;
          setErrorMsg(errs ? Object.entries(errs).map(([f, m]) => `${f}: ${(m as string[]).join(', ')}`).join('\n') : message);
  ...
  const handleDelete = async (id: string, name: string) => {
    ...
    try { await productApi.delete(id); refetch(); showToast('Produk dihapus'); }
    ...
    <div className="min-h-screen" style={{ background: '#f4f6f9', fontFamily: "'DM Sans', -apple-system, sans-serif" }}>
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <div className="flex items-center justify-center flex-shrink-0"
                style={{ width: 36, height: 36, borderRadius: 10, background: 'linear-gradient(135deg, #0891b2 0%, #0ea5e9 100%)', boxShadow: '0 4px 12px rgba(8,145,178,0.28)' }}>
              ...
              <div>
                <h1 className="font-bold text-slate-900" style={{ fontSize: 17, lineHeight: 1.2 }}>Produk</h1>
            ...
            <PermissionGate permission="data.product.create">
              <button onClick={() => { setModalOpen(true); setErrorMsg(''); }}
                ...
                style={{ padding: '8px 16px', fontSize: 13.5, background: 'linear-gradient(135deg, #0891b2 0%, #0ea5e9 100%)', boxShadow: '0 4px 14px rgba(8,145,178,0.3)', border: 'none', cursor: 'pointer', fontFamily: 'inherit' }}
                onMouseEnter={(e) => (e.currentTarget as HTMLElement).style.boxShadow = '0 6px 18px rgba(8,145,178,0.42)'}
                onMouseLeave={(e) => (e.currentTarget as HTMLElement).style.boxShadow = '0 4px 14px rgba(8,145,178,0.3)'}>
      ...
      <div className="px-6 md:px-8 py-6" style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div className="bg-white rounded-xl overflow-hidden" style={{ border: '1px solid #e9ecef', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
          <div className="flex items-center justify-between px-4 py-2.5" style={{ borderBottom: '1px solid #f1f5f9', background: '#f8fafc' }}>
            <div className="flex items-center gap-2">
              ...
              {hasFilter && <span style={{ fontSize: 10.5, fontWeight: 700, padding: '2px 8px', borderRadius: 99, background: '#ecfeff', color: '#155e75', border: '1px solid #a5f3fc' }}>Aktif</span>}
            </div>
            {hasFilter && (
          ...
          <div className="flex flex-wrap gap-3 p-3.5">
            <div className="relative flex-1" style={{ minWidth: 200 }}>
              ...
              <input type="text" placeholder="Cari nama atau kode produk..." value={search}
                ...
                onFocus={(e) => { e.currentTarget.style.borderColor = '#0891b2'; e.currentTarget.style.boxShadow = '0 0 0 3px rgba(8,145,178,0.1)'; }}
        ...
        <div className="bg-white rounded-xl overflow-hidden" style={{ border: '1px solid #e9ecef', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
          {isLoading ? (
            ...
            <div className="text-center py-16 px-6">
              <div className="flex items-center justify-center mx-auto mb-4"
                style={{ width: 52, height: 52, borderRadius: 14, background: '#ecfeff', border: '1px solid #a5f3fc' }}>
                <svg className="w-6 h-6" style={{ color: '#0891b2' }} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M20.25 7.5l-.625 10.632a2.25 2.25 0 01-2.247 2.118H6.622a2.25 2.25 0 01-2.247-2.118L3.75 7.5M10 11.25h4M3.375 7.5h17.25c.621 0 1.125-.504 1.125-1.125v-1.5c0-.621-.504-1.125-1.125-1.125H3.375c-.621 0-1.125.504-1.125 1.125v1.5c0 .621.504 1.125 1.125 1.125z" />
                </svg>
              </div>
              <p className="font-semibold" style={{ fontSize: 14, color: '#475569' }}>Belum ada produk</p>
              <p style={{ fontSize: 12.5, color: '#94a3b8', marginTop: 5 }}>
                {hasFilter ? 'Tidak ada hasil untuk filter ini.' : 'Klik "Tambah Produk" untuk menambahkan data.'}
              </p>
            </div>
          ) : (
            <>
              <div style={{ padding: '8px 16px', background: '#f8fafc', borderBottom: '1px solid #f1f5f9' }}>
                <span style={{ fontSize: 11, color: '#94a3b8', fontWeight: 500 }}>
                  ...
                  <strong style={{ color: '#475569' }}>{pagination?.total ?? data.data.length}</strong> produk
                </span>
              </div>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  ...
                  <tbody>
                    {data.data.map((prod: Product) => {
                      const tc = TYPE_CONFIG[prod.type ?? ''] ?? { label: prod.type ?? '—', bg: '#f8fafc', text: '#475569', border: '#e2e8f0', dot: '#94a3b8' };
                      return (
                        // FIX 1: tambah className="group" agar group-hover pada action buttons bekerja
                        <tr key={prod.id} className="group"
                          ...
                          <td style={{ padding: '12px 16px' }}>
                            <div>
                              <p style={{ fontSize: 13.5, fontWeight: 600, color: '#1e293b' }}>{prod.name}</p>
                              {prod.description && <p style={{ fontSize: 12, color: '#94a3b8', marginTop: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 220 }}>{prod.description}</p>}
                            </div>
                          </td>
                          ...
                          <td style={{ padding: '12px 16px' }}>
                            {prod.code
                              ? <span style={{ fontSize: 12, fontWeight: 600, fontFamily: 'ui-monospace, monospace', padding: '3px 8px', borderRadius: 6, background: '#ecfeff', color: '#155e75', border: '1px solid #a5f3fc' }}>{prod.code}</span>
                          ...
                          {/* Unit */}
                          <td style={{ padding: '12px 16px', fontSize: 13, color: '#475569' }}>{prod.unit || '—'}</td>

                          {/* Price */}
                          <td style={{ padding: '12px 16px', textAlign: 'right' }}>
                            {prod.price
                              ? <span style={{ fontSize: 13.5, fontWeight: 600, color: '#0f172a' }}>{fmtIDR(Number(prod.price))}</span>
                              : <span style={{ fontSize: 12.5, color: '#cbd5e1' }}>—</span>}
                          </td>
                          ...
                          <td style={{ padding: '12px 16px' }}>
                            <span className="inline-flex items-center gap-1.5"
                              ...
                              <span style={{ width: 6, height: 6, borderRadius: '50%', background: prod.isActive ? '#10b981' : '#94a3b8' }} />
                          ...
                          <td style={{ padding: '12px 16px' }}>
                            <div className="flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                              <PermissionGate permission="data.product.view">
          ...
          {/* FIX 2: Smart pagination dengan ellipsis */}
      ...
      {modalOpen && (
        <ModalShell title="Tambah Produk Baru" subtitle="Isi detail produk di bawah ini" onClose={() => { setModalOpen(false); setErrorMsg(''); }}>
          ...
          <ProductForm onSubmit={handleCreate} onCancel={() => { setModalOpen(false); setErrorMsg(''); }} isLoading={isCreating} />
        </ModalShell>
      )}
    </div>
  );
