// FILE: src/modules/operations/stock/components/StockOutForm.tsx
import { useEffect, useState } from 'react';
import { useForm } from 'react-hook-form';
import { zodResolver } from '@hookform/resolvers/zod';
import { productApi } from '@/shared/api/product.api';
import { warehouseApi } from '@/shared/api/warehouse.api';
import { stockOutSchema, StockOutFormData } from '../stock.schema';
...
export const StockOutForm = ({ onSubmit, onCancel, isLoading = false }: StockOutFormProps) => {
  const [products,    setProducts]   = useState<Product[]>([]);
  const [warehouses,  setWarehouses] = useState<Warehouse[]>([]);
  const [loadingData, setLoadingData] = useState(true);

  const { register, handleSubmit, formState: { errors } } = useForm<StockOutFormData>({
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}
      ...
      <div>
        ...
        <StyledInput {...register('referenceNo')} placeholder="DO-001" disabled={disabled} />
