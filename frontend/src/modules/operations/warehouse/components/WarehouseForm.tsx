// FILE: src/modules/operations/warehouse/components/WarehouseForm.tsx
import { useForm } from 'react-hook-form';
import { zodResolver } from '@hookform/resolvers/zod';
import { warehouseSchema, WarehouseFormData } from '../warehouse.schema';
...
export const WarehouseForm = ({ warehouse, onSubmit, onCancel, isLoading = false }: WarehouseFormProps) => {
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <FieldLabel>Nama Gudang <span style={{ color: '#ef4444' }}>*</span></FieldLabel>
          <StyledInput {...register('name')} placeholder="cth. Gudang Utama" disabled={isLoading} />
          <FieldError msg={errors.name?.message} />
        </div>
        <div>
          <FieldLabel>Kode Gudang</FieldLabel>
          <StyledInput {...register('code')} placeholder="cth. WH001" disabled={isLoading} />
      ...
      <div>
        ...
        <StyledTextarea {...register('location')} rows={3} placeholder="Alamat lengkap gudang..." disabled={isLoading} />
      ...
      <div className="flex items-center justify-end gap-2.5" style={{ paddingTop: 12, borderTop: '1px solid #f1f5f9', marginTop: 4 }}>
        ...
        >
          ...
          {isLoading ? 'Menyimpan...' : warehouse ? 'Simpan Perubahan' : 'Buat Gudang'}
        </button>
      </div>
    </form>
  );
