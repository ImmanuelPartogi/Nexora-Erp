// FILE: frontend/src/modules/operations/lease/pages/LeaseListPage.tsx
...
export const LeaseListPage = () => {
  ...
  const { data, isLoading, refetch } = useLeases({ page, limit: 10, search, status: status || undefined });
  const showToast = (msg: string) => { setToast(msg); setTimeout(() => setToast(null), 3000); };

  const handleCreate = async (formData: LeaseFormData) => {
    try {
      setIsCreating(true);
      await leaseApi.create(formData);
      setModalOpen(false); refetch();
      showToast('Sewa berhasil dibuat');
    } catch { alert('Gagal membuat sewa. Coba lagi.'); }
    finally { setIsCreating(false); }
  };

  const handleApprove = async (id: string, customerName?: string) => {
    if (!confirm(`Setujui sewa untuk "${customerName}"?`)) return;
    try { await leaseApi.approve(id); refetch(); showToast('Sewa berhasil disetujui'); }
        ...
        <div className="px-6 md:px-8 py-5">
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="font-bold text-slate-900" style={{ fontSize: 17, lineHeight: 1.2 }}>Sewa</h1>
        ...
        <div className="bg-white rounded-xl overflow-hidden" style={{ border: '1px solid #e9ecef', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
          ...
          <div className="flex flex-wrap gap-3 p-3.5">
            ...
            <div className="relative flex-1" style={{ minWidth: 200 }}>
              ...
              <input type="text" placeholder="Cari nama customer atau unit..." value={search}
        ...
        <div className="bg-white rounded-xl overflow-hidden" style={{ border: '1px solid #e9ecef', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
          {isLoading ? (
            ...
            <div className="text-center py-16 px-6">
              ...
              <p className="font-semibold" style={{ fontSize: 14, color: '#475569' }}>Belum ada sewa</p>
              <p style={{ fontSize: 12.5, color: '#94a3b8', marginTop: 5 }}>
                {hasFilter ? 'Tidak ada hasil untuk filter ini.' : 'Klik "Tambah Sewa" untuk membuat perjanjian baru.'}
              </p>
            </div>
          ) : (
            <>
              <div style={{ padding: '8px 16px', background: '#f8fafc', borderBottom: '1px solid #f1f5f9' }}>
                <span style={{ fontSize: 11, color: '#94a3b8', fontWeight: 500 }}>
                  ...
                  <strong style={{ color: '#475569' }}>{pagination?.total ?? data.data.length}</strong> sewa
                </span>
              </div>
              <div className="overflow-x-auto">
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead>
                    <tr><TH>Customer</TH><TH>Unit</TH><TH>Periode</TH><TH>Jumlah</TH><TH>Status</TH><TH>Aksi</TH></tr>
                  </thead>
                  <tbody>
                      ...
                      return (
                        <tr key={lease.id} className="group"
                          ...
                          <td style={{ padding: '12px 16px' }}>
                            <p style={{ fontSize: 13.5, fontWeight: 600, color: '#1e293b' }}>{lease.customerName}</p>
                          </td>

                          {/* Unit */}
                          <td style={{ padding: '12px 16px', fontSize: 13, color: '#475569' }}>
                            {lease.unitName || lease.unitId || '—'}
                          </td>
                          ...
                          <td style={{ padding: '12px 16px' }}>
                            ...
                            <p style={{ fontSize: 11.5, color: '#94a3b8', marginTop: 2 }}>{days} hari</p>
                          </td>

                          {/* Jumlah */}
                          <td style={{ padding: '12px 16px' }}>
                            <span style={{ fontSize: 13.5, fontWeight: 600, color: '#0f172a' }}>{fmtIDR(Number(lease.amount))}</span>
                          </td>
                          ...
                          <td style={{ padding: '12px 16px' }}>
                            <div className="flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                              <button onClick={() => navigate(`/leases/${lease.id}`)}
